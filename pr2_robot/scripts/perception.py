...
# Helper function to get world number
def get_world():
    world = 1
    xmldoc = minidom.parse('../launch/pick_place_project.launch')
    nodelist = xmldoc.getElementsByTagName('arg')
    for node in nodelist:
        if node.attributes['name'].value == 'world_name':
            if node.attributes['value'].value == '$(find pr2_robot)/worlds/test2.world':
                world = 2
            if node.attributes['value'].value == '$(find pr2_robot)/worlds/test3.world':
                world = 3
...
def pcl_callback(pcl_msg):
    ...
    #Y filter to eliminate lateral boxes
    passthrough = cloud_filtered.make_passthrough_filter()
    ...
    #print ("Objetos: ",len(cluster_indices))
...
def pr2_mover(object_list):
    ...
    test_scene_num.data = int(get_world())
    ...
    for i in range(0, len(object_list_param)):
        ...
        for j in range(0, len(labels)):
            if labels[j] == object_list_param[i]['name']:
                #Pick pose Change to the corresponding centroid
        ...
        for j in range(0, len(dropbox_list_param)):
            if dropbox_list_param[j]['group'] == object_list_param[i]['group']:
    ...
    send_to_yaml('result' + str(test_scene_num.data) + '.yaml', dict_list)
